...
# $ wget https://raw.githubusercontent.com/python-poetry/poetry/1.1.15/get-poetry.py
...
RUN useradd rca --create-home && mkdir /app $VIRTUAL_ENV && chown -R rca /app $VIRTUAL_ENV
...
# Run poetry install again to install our project (so the the rca package is always importable)
...
# Load shortcuts
COPY ./docker/bashrc.sh /home/rca/.bashrc
